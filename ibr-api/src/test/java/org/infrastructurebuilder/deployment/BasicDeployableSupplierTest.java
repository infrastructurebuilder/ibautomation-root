...
public class BasicDeployableSupplierTest {
  ...
  public void testAbstractDeployableSupplier() {
    new BasicTestDeployableSupplier(root, wps.get());
    Path p = wps.get();
    s = new BasicTestDeployableSupplier(root, p);
    assertEquals(p.relativize(root), s.getRootFile());
    assertEquals(root, s.get());

  }

  @Test(expected = IBDeploymentException.class)
  public void testAbstractDeployableSupplierBadMainFile() {
    new BasicTestDeployableSupplier(root, root.getParent());
  }
