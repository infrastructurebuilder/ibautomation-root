...
public class IBRContainerDialectSupplier extends AbstractIBRDialectSupplier {
  ...
  @Inject
  public IBRContainerDialectSupplier(AutomationUtils ibr, List<IBRInstanceType> sizings) {
    super(ibr, TYPE);
    this.imageSizes = requireNonNull(sizings).stream().filter(t -> t.respondsToDialect(TYPE)).collect(toList());
  }

  private IBRContainerDialectSupplier(AutomationUtils ibr, ConfigMapSupplier cms, List<IBRInstanceType> isizes,
      int weight) {
  ...
  public IBRDialectSupplier configure(ConfigMapSupplier cms) {
    return new IBRContainerDialectSupplier(getAutomationUtils(), cms, this.imageSizes, getWeight());
  }
